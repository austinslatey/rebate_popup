...
export async function sendQuoteEmail({ first_name, last_name, email, phone, product_title, collection_handle, variant_id }) {
  if (!process.env.SENDGRID_API_KEY) {
    console.error("Error: SENDGRID_API_KEY is missing!");
    return false;
  }
    ...
    const msg = {
      ...
      html: `
          ...
          <ul>
              ...
              <li><strong>Variant ID:</strong> ${variant_id || 'Not specified'}</li>
          </ul>
          <p>Thank you,</p>
          <p>The Waldoch Team</p>
          ${WALDOCH_LOGO}`,
